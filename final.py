...
class Application:
    ...
    def __init__(self):
        ...
        self.panel.place(x=95, y=2, width=450, height=600)

        self.panel2 = tk.Label(self.root)  # initialize image panel
        # self.panel2.place(x=700, y=115, width=400, height=400)
        self.panel2.place(x=650, y=110, width=350, height=350)
        ...
        # Start button
        # self.start_button = tk.Button(text="🟢 Start Transcription", font=("Helvetica", 14), command=self.start_transcription)
        # self.start_button.pack(pady=10)

        # # Stop button
        # self.stop_button = tk.Button(text="🔴 Stop Transcription", font=("Helvetica", 14), command=self.stop_transcription)
        # self.stop_button.pack(pady=5)
        self.start_button = tk.Button(self.root, text="🟢 Start Transcription", font=("Helvetica", 14), command=self.start_transcription)
        self.start_button.place(x=1205, y=680)

        self.stop_button = tk.Button(self.root, text="🔴 Stop Transcription", font=("Helvetica", 14), command=self.stop_transcription)
        self.stop_button.place(x=1205, y=720)

        # Text box
        # self.text_box = scrolledtext.ScrolledText(wrap=tk.WORD, width=60, height=15, font=("Consolas", 12))
        # self.text_box.pack(padx=10, pady=10)

        # # Status label
        self.status_label = tk.Label(text="Ready", font=("Helvetica", 12), fg="green")
        self.status_label.pack()
        self.transcription_label = tk.Label(self.root, text="Transcription Output:", font=("Helvetica", 14))
        self.transcription_label.place(x=10, y=650)

        self.text_box = scrolledtext.ScrolledText(self.root, wrap=tk.WORD, width=100, height=6, font=("Consolas", 12))
        self.text_box.place(x=10, y=650)
    ...
    def callback(self, recognizer, audio):
        try:
            ...
            self.status_label.config(text="Didn't catch that.")
        except sr.RequestError as e:
            self.status_label.config(text=f"Error: {e}")
        except Exception as e:
            self.status_label.config(text=f"Exception: {e}")

    def start_transcription(self):
        if not self.listening:
            self.status_label.config(text="Listening...")
            self.listening = True
            self.stop_listening = self.recognizer.listen_in_background(sr.Microphone(), self.callback)
        else:
            self.status_label.config(text="Already listening.")

    def stop_transcription(self):
        if self.listening and self.stop_listening:
